# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  chatgpt-desktop = {
    pname = "chatgpt-desktop";
    version = "26.820.80927";
    src = fetchurl {
      url = "https://persistent.oaistatic.com/codex-app-prod/linux/deb/pool/main/c/chatgpt/chatgpt_26.820.80927_amd64.deb";
      sha256 = "sha256-Ce/c0GUf2iqqNFehsJKVBKXejrzE5intjP6+NOmKSfg=";
    };
  };
  flate = {
    pname = "flate";
    version = "0.6.1";
    src = fetchurl {
      url = "https://github.com/home-operations/flate/releases/download/v0.6.1/flate_0.6.1_linux_amd64.tar.gz";
      sha256 = "sha256-Jd5l3U9EeFdXaLTDDWutTLt6daeji+WjzXdq4GX9ZjE=";
    };
  };
  kubectl-kopiur = {
    pname = "kubectl-kopiur";
    version = "0.10.4";
    src = fetchurl {
      url = "https://github.com/home-operations/kopiur/releases/download/0.10.4/kubectl-kopiur_0.10.4_linux_amd64.tar.gz";
      sha256 = "sha256-QZvPgxOo0nZeQKxLrIvDJNUv4pqySZ7ptsr40nuSzwk=";
    };
  };
  wagoapp = {
    pname = "wagoapp";
    version = "2.14.1";
    src = fetchurl {
      url = "https://cdn.wago.io/wagoapp/WagoApp_2.14.1.AppImage";
      name = "WagoApp_2.14.1.AppImage";
      sha256 = "sha256-QUXUwFrV3Ysv3QZJJ8QUvP8RRaiR62zw3S0sXRGtGG4=";
    };
  };
}
